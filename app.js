...
var processBLegs = function(legInfo, cdrListArr, callback) {
  ...
  if (
    legType &&
    (legType === "ATT_XFER_USER" || legType === "ATT_XFER_GATEWAY")
  ) {
  ...
  dbHandler.GetBLegsForIVRCalls(uuid, callUuid, function(err, legInfo) {
    if (legInfo && legInfo.length > 0) {
      ...
      async.parallel(asyncArr, function(err, missingOriginatedLegs) {
        var cleanedUpList = missingOriginatedLegs.filter(function(missingLeg) {
          return missingLeg;
        });
        cleanedUpList.push("CALL_UUID_" + callUuid);

        redisHandler.GetSetMembers("UUID_MAP_" + uuid, function(
          err,
          missingLegsRedis
        ) {
          if (missingLegsRedis && missingLegsRedis.length > 0) {
            ...
            if (!allItemsAlreadyAdded) {
              //ADD TO REDIS
              addRedisObjects(uuid, cleanedUpList, function(
                err,
                redisAddResult
              ) {
                //CALLBACK
...
var collectOtherLegsCDR = function(
  cdrListArr,
  relatedLegs,
  tryCount,
  mainLegId,
  ...
  var actionObj = {};
  dbHandler.GetSpecificLegsByUuids(relatedLegs, function(
    allLegsFound,
    objList
  ) {
    if (allLegsFound) {
      ...
      async.parallel(asyncArr, function(err, missingLegsList) {
        if (missingLegsList) {
          ...
          var cleanedUpList = missingLegsList.filter(function(missingLeg) {
            return missingLeg;
          });
          ...
          if (cleanedUpList && cleanedUpList.length > 0) {
            ...
            redisHandler.GetSetMembers("UUID_MAP_" + mainLegId, function(
              err,
              missingLegsRedis
            ) {
              if (missingLegsRedis && missingLegsRedis.length > 0) {
                ...
                if (!allItemsAlreadyAdded) {
                  //ADD TO REDIS
                  addRedisObjects(mainLegId, cleanedUpList, function(
                    err,
                    redisAddResult
                  ) {
                    //CALLBACK
...
var processCDRLegs = function(processedCdr, cdrList, callback) {
  ...
  if (processedCdr.RelatedLegs && relatedLegsLength) {
    ...
    collectOtherLegsCDR(
      cdrList[processedCdr.Uuid],
      processedCdr.RelatedLegs,
      processedCdr.TryCount,
      processedCdr.Uuid,
    ...
    if (
      processedCdr.ObjType === "HTTAPI" ||
      processedCdr.ObjType === "SOCKET" ||
      processedCdr.ObjCategory === "DIALER"
    ) {
      console.log("=============== HTTAPI LEGS PROCESSING =================");
      collectBLegs(
        cdrList[processedCdr.Uuid],
        processedCdr.Uuid,
        processedCdr.CallUuid,
        function(err, actionObj) {
          callback(null, cdrList, actionObj);
        }
      );
      ...
      console.log(
        "================== UNKNOWN TYPE LEG CALL FOUND PROCESSING AS IT IS - UUID : " +
          processedCdr.Uuid +
          " =================="
      );
...
var processCampaignCDR = function(primaryLeg, curCdr) {
  ...
  if (firstLeg) {
    ...
    if (
      firstLeg.ObjType === "ATT_XFER_USER" ||
      firstLeg.ObjType === "ATT_XFER_GATEWAY"
    ) {
    ...
    if (
      firstLeg.ObjType === "BLAST" ||
      firstLeg.ObjType === "DIRECT" ||
      firstLeg.ObjType === "IVRCALLBACK"
    ) {
  ...
  if (
    callHangupDirectionA === "recv_bye" ||
    callHangupDirectionA === "recv_refuse"
  ) {
    cdrAppendObj.HangupParty = "CALLEE";
  } else if (
    callHangupDirectionB === "recv_bye" ||
    callHangupDirectionB === "recv_refuse"
  ) {
...
var processSingleCdrLeg = function(primaryLeg, callback) {
  ...
  processCDRLegs(cdr, cdrList, function(err, resp, actionObj) {
    if (err) {
      logger.error(
        "[DVP-CDRProcessor.processSingleCdrLeg] - [%s] - Error occurred while processing CDR Legs",
        err
      );
    }
    if (actionObj.SaveOnDB) {
      ...
      if (
        primaryLeg.DVPCallDirection === "outbound" &&
        (primaryLeg.ObjType === "ATT_XFER_USER" ||
          primaryLeg.ObjType === "ATT_XFER_GATEWAY")
      ) {
        isOutboundTransferCall = true;
      }
      ...
      if (cdr.ObjCategory === "DIALER") {
        ...
        filteredOutb.reduce(function(accumilator, currentValue) {
          if (isOutboundTransferCall) {
            if (
              currentValue.ObjType !== "ATT_XFER_USER" &&
              currentValue.ObjType !== "ATT_XFER_GATEWAY"
            ) {
            ...
            if (
              (currentValue.ObjType === "ATT_XFER_USER" ||
                currentValue.ObjType === "ATT_XFER_GATEWAY") &&
              !currentValue.IsTransferredParty
            ) {
              accumilator.transferLegB.push(currentValue);
            }
        ...
        if (
          transLegInfo &&
          transLegInfo.actualTransferLegs &&
          transLegInfo.actualTransferLegs.length > 0 &&
          transLegInfo.transferLegB &&
          transLegInfo.transferLegB.length > 0
        ) {
          transLegInfo.actualTransferLegs.forEach(function(actualTransLeg) {
            var index = transLegInfo.transferLegB
              .map(function(e) {
                return e.Uuid;
              })
              .indexOf(actualTransLeg.Uuid);
        ...
        if (transLegInfo.transferLegB && transLegInfo.transferLegB.length > 0) {
          var transferLegBAnswered = transLegInfo.transferLegB.filter(function(
            item
          ) {
        ...
        if (transferCallOriginalCallLeg) {
          ...
          for (k = 0; k < transLegInfo.actualTransferLegs.length; k++) {
            transferredParties =
              transferredParties +
              transLegInfo.actualTransferLegs[k].SipToUser +
              ",";
          }
        } else {
          if (filteredOutb.length > 1) {
            ...
            if (filteredOutbAnswered && filteredOutbAnswered.length > 0) {
              if (filteredOutbAnswered.length > 1) {
                ...
                var filteredOutbBridged = filteredOutbAnswered.filter(function(
                  item3
                ) {
            ...
            if (filteredOutb && filteredOutb.length > 0) {
              ...
              if (
                callCategory === "FOLLOW_ME" ||
                callCategory === "FORWARDING"
              ) {
                for (k = 0; k < filteredOutb.length; k++) {
                  transferredParties =
                    transferredParties + filteredOutb[k].SipToUser + ",";
                }
              }
            }
          }
        ...
        if (secondaryLeg) {
          ...
          if (cdrAppendObj.DVPCallDirection === "inbound" && outLegAnswered) {
            cdrAppendObj.BillSec =
              primaryLeg.Duration - primaryLeg.TimeAfterInitialBridge;
          }
        ...
        cdrAppendObj.IvrConnectSec =
          cdrAppendObj.Duration - cdrAppendObj.QueueSec - cdrAppendObj.BillSec;

        cdrAppendObj.IsAnswered = outLegAnswered;

        if (
          callHangupDirectionA === "recv_bye" ||
          callHangupDirectionA === "recv_cancel"
        ) {
          cdrAppendObj.HangupParty = "CALLER";
        } else if (
          callHangupDirectionB === "recv_bye" ||
...
connection.on("connect", function() {
  logger.debug(
    "[DVP-CDRProcessor.AMQPConnection] - [%s] - AMQP Connection CONNECTED"
  );
});
...
connection.on("ready", function() {
  ...
  logger.debug(
    "[DVP-CDRProcessor.AMQPConnection] - [%s] - AMQP Connection READY"
  );

  readyCount += 1;
  if (readyCount === 1)
    connection.queue("CDRQUEUE", { durable: true, autoDelete: false }, function(
      q
    ) {
      q.bind("#");
      ...
      q.subscribe(function(message) {
        ...
        processSingleCdrLeg(message, function(err, actionObj) {
          if (actionObj.RemoveFromRedis) {
            ...
            redisHandler.GetSetMembers("UUID_MAP_" + message.Uuid, function(
              err,
              items
            ) {
